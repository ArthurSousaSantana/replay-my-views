      ...
      <div className="bg-surface rounded-xl shadow-md border border-border flex flex-col h-full overflow-hidden">
        <div className="relative h-40 bg-white overflow-hidden flex items-center justify-center">
          <div className="absolute top-2 left-2 z-10">
            <span className={`${badgeColor} text-[10px] font-bold px-2 py-0.5 rounded-md backdrop-blur-sm uppercase tracking-wider`}>{badgeText}</span>
          </div>
          <img alt={title} className="max-w-full max-h-full object-contain p-3" src={image} />
        </div>
        <div className="p-3 flex-1 flex flex-col">
          ...
          <div className="mt-auto">
            ...
            <div className="flex justify-between items-center mb-3">
              <span className="text-lg font-bold text-foreground">{newPrice}</span>
              <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-1.5 py-0.5 rounded">{discount}</span>
            </div>
            <Link to={link} className="w-full bg-secondary hover:opacity-90 text-secondary-foreground text-xs font-semibold py-2 rounded-lg flex items-center justify-center gap-1 transition-colors shadow-sm">
