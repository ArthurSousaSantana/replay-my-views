...
const BuildCard = ({ title, image, badgeText, badgeColor, oldPrice, newPrice, discount, description, chips = [], link = "/builds/1", compact = false }: BuildCardProps) => {
  if (compact) {
    return (
      <Link to={link} className="bg-surface rounded-xl shadow-md border border-border flex flex-col h-full overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <div className="p-2.5 flex-1 flex flex-col">
          ...
          {description && (
            <p className="text-[10px] text-muted-foreground mb-1 line-clamp-2 leading-tight">{description}</p>
          )}
  ...
  return (
    <Link to={link} className="bg-surface rounded-xl shadow-lg hover:shadow-xl transition-shadow overflow-hidden group border border-border flex flex-col h-full">
