...
const Pagination = ({ currentPage = 1, totalPages = 3, showInfo, infoText }: PaginationProps) => {
  ...
  return (
    <div className={showInfo ? "bg-muted/30 px-6 py-4 border-t border-border flex items-center justify-between" : "mt-12 flex justify-center"}>
      ...
      <nav className="flex items-center gap-1">
        ...
        {showLastPage && totalPages > 3 && (
          <button className="flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-lg border border-border text-xs font-medium text-foreground hover:bg-muted transition-colors bg-surface">
            {totalPages}
          </button>
        )}
        <button className="flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-lg border border-border bg-surface text-muted-foreground hover:bg-muted transition-colors">
