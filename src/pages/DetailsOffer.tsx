...
const relatedOffers = [
  { title: "Samsung Galaxy Tab S9", image: IMG_TABLET, badge: "-10% OFF", oldPrice: "A partir de R$ 6.999", newPrice: "R$ 4.199,00", discount: "+31%" },
  { title: "AirPods Pro (2ª geração)", icon: "headphones", oldPrice: "A partir de R$ 2.599", newPrice: "R$ 1.899,00", discount: "+27%" },
  { title: "Apple Watch Series 9", icon: "watch", oldPrice: "A partir de R$ 4.999", newPrice: "R$ 3.599,00", discount: "+28%" },
  { title: "Carregador 20W USB-C", icon: "charger", oldPrice: "A partir de R$ 219", newPrice: "R$ 159,00", discount: "+27%" },
];
...
const DetailsOffer = () => {
  return (
    <div className="bg-background text-foreground">
      ...
      <header className="relative overflow-hidden min-h-[200px] flex items-center justify-center text-white hero-gradient">
        ...
        <div className="container mx-auto px-4 z-20 relative pt-8 pb-8">
          <nav aria-label="Breadcrumb" className="flex mb-4 text-sm font-medium text-blue-200">
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              <li><Link to="/" className="inline-flex items-center hover:text-white"><span className="material-symbols-outlined text-sm mr-2">home</span>Home</Link></li>
              <li><div className="flex items-center"><span className="material-symbols-outlined text-sm mx-1">chevron_right</span><Link to="/ofertas" className="hover:text-white">Smartphones</Link></div></li>
              <li aria-current="page"><div className="flex items-center"><span className="material-symbols-outlined text-sm mx-1">chevron_right</span><span className="text-white">Apple</span></div></li>
            </ol>
          </nav>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex items-center justify-between gap-2 text-sm text-muted-foreground">
                  ...
                  <div className="flex gap-2">
                    {[
                      { icon: "chat", color: "text-green-500", bg: "bg-green-50 hover:bg-green-100" },
                      { icon: "send", color: "text-blue-500", bg: "bg-blue-50 hover:bg-blue-100" },
                      { icon: "post", color: "text-sky-500", bg: "bg-sky-50 hover:bg-sky-100" },
                      { icon: "content_copy", color: "text-muted-foreground", bg: "bg-muted hover:opacity-80" },
                    ].map((btn) => (
                      <button key={btn.icon} className={`w-10 h-10 rounded-full ${btn.bg} ${btn.color} flex items-center justify-center transition-colors`}>
        ...
        <section className="mt-16 pt-8 border-t border-border">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedOffers.map((offer) => (
              <div key={offer.title} className="bg-surface rounded-xl shadow-sm hover:shadow-md transition-all border border-border p-4 flex flex-col">
                <div className="relative mb-4 bg-card rounded-lg p-6 flex items-center justify-center h-48">
                  {offer.badge && (
                    <div className="absolute top-2 left-2">
                      <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded">{offer.badge}</span>
                    </div>
                  )}
                  {offer.image ? (
                    <img alt={offer.title} className="max-h-full max-w-full object-contain" src={offer.image} />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-muted rounded text-muted-foreground">
                      <span className="material-symbols-outlined text-6xl">{offer.icon}</span>
                    </div>
                  )}
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-foreground mb-1">{offer.title}</h3>
                  <p className="text-xs text-muted-foreground mb-2 line-through">{offer.oldPrice}</p>
