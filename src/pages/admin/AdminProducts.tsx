    ...
    <div className="min-h-screen bg-background">
      ...
      <HeroBanner size="md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <nav className="flex items-center gap-2 text-xs text-blue-200/70 mb-3">
              <span>Admin</span>
          ...
          <Link
            ...
            className="bg-white text-primary hover:bg-blue-50 font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 shadow-xl transition-all transform hover:scale-[1.02] active:scale-95"
          >
      ...
      <main className="container mx-auto px-4 -mt-12 relative z-20 pb-20">
        ...
        <div className="bg-card rounded-2xl shadow-xl overflow-hidden border border-border">
              ...
              <thead>
                <tr className="bg-muted/50 border-b border-border">
                  {["Produto", "Categoria", "Preço", "Status", "Ações"].map((h, i) => (
                    <th key={h} className={`px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-wider ${i === 4 ? "text-right" : ""}`}>
                      {h}
                    </th>
                  ))}
                </tr>
          ...
          <div className="bg-muted/30 px-6 py-4 border-t border-border flex items-center justify-between">
            <p className="text-xs text-muted-foreground">Exibindo <span className="font-bold">1-10</span> de <span className="font-bold">254</span> produtos</p>
            <div className="flex items-center gap-1">
              <button className="w-8 h-8 rounded border border-border flex items-center justify-center text-muted-foreground hover:bg-card transition-colors disabled:opacity-50" disabled>
                <span className="material-symbols-outlined text-sm">chevron_left</span>
              </button>
              <button className="w-8 h-8 rounded bg-primary text-primary-foreground text-xs font-bold">1</button>
              {[2, 3].map((n) => (
                <button key={n} className="w-8 h-8 rounded border border-border text-xs font-medium text-muted-foreground hover:bg-card transition-colors">{n}</button>
              ))}
              <span className="text-muted-foreground px-1">...</span>
              <button className="w-8 h-8 rounded border border-border text-xs font-medium text-muted-foreground hover:bg-card transition-colors">25</button>
              <button className="w-8 h-8 rounded border border-border flex items-center justify-center text-muted-foreground hover:bg-card transition-colors">
