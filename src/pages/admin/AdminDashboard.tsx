      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button className="bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white font-semibold py-2.5 px-6 rounded-lg flex items-center gap-2 shadow-lg transition-all hover:border-white/40 text-sm">
            <span className="material-symbols-outlined text-lg">category</span>Nova Categoria
          </button>
        </div>
      </div>
    </HeroBanner>
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-card rounded-xl shadow-lg border border-border p-6">
            ...
            <div className="flex flex-col items-center">
              <div className="w-40 h-40 rounded-full mb-6 relative" style={{ background: "conic-gradient(hsl(var(--primary)) 0% 45%, hsl(var(--accent)) 45% 75%, #F59E0B 75% 90%, #EF4444 90% 100%)" }}>
          ...
          <div className="bg-card rounded-xl shadow-lg border border-border p-6">
            <h3 className="text-lg font-bold text-foreground mb-6 flex items-center gap-2">
              <span className="material-symbols-outlined text-primary">bar_chart</span>Builds por Tipo
            </h3>
            <div className="flex items-end h-40 gap-3 pb-6 border-b border-border">
              {barData.map((bar) => (
                <div key={bar.label} className={`flex-1 ${bar.color} rounded-t relative hover:opacity-80 transition-opacity`} style={{ height: bar.height }}>
                  <span className="absolute bottom-[-24px] left-1/2 -translate-x-1/2 text-[10px] text-muted-foreground whitespace-nowrap">{bar.label}</span>
                </div>
              ))}
            </div>
                ...
                <div key={build.name} className="p-4 flex items-center gap-4 hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${build.statusColor}`}>{build.status}</span>
                    </div>
